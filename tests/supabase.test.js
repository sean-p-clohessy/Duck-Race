...
test('staff function and password update use the signed-in staff session',async()=>{const calls=setup([{body:session()},{body:{staff:[]}},{body:{id:'staff-id'}}]);const client=createClient('https://example.supabase.co','public-key');await client.auth.signInWithPassword({email:'e',password:'p'});await client.functions.invoke('manage-staff',{body:{action:'list'}});await client.auth.updateUser({password:'new-password'});assert(calls[1].url.endsWith('/functions/v1/manage-staff'));assert.equal(calls[1].options.headers.Authorization,'Bearer test-access');assert.deepEqual(JSON.parse(calls[1].options.body),{action:'list'});assert.equal(calls[2].options.method,'PUT');assert(calls[2].url.endsWith('/auth/v1/user'));});
